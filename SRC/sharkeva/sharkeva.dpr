...
begin
  ...
  SetRetResult('f.sharkeva.com/|1036'); // С какого сервака умеет. Подстрока.
...
function AnErrorOnPage(text:string):boolean;
	...
	result:=true;
	// файл удален
	if pos('Неверная ссылка', text) > 0 then
		begin
...
begin
  ...
  DeleteFile(WorkDir+'sharkeva1.html');
  DeleteFile(WorkDir+'sharkeva2.html');
   s:=Get(Lnk);
    if FDebug then
      SaveToFile(PChar(WorkDir + 'sharkeva1.html'), PChar(s));
  if AnErrorOnPage(s) then exit;
     ...
     if s1<>'' then
                ...
                then WaitTime(t);
    if FDebug then
      SaveToFile(PChar(WorkDir + 'sharkeva2.html'), PChar(s));
                end;
	randomize;
   SetRetResult(PChar(Lnk + '|' + (IntToStr(random(5)))));
  end;
