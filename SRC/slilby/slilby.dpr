...
begin
  ...
  SetRetResult('lil.by/files|1036'); // С какого сервака умеет. Подстрока.
...
function AnErrorOnPage(text:string):boolean;
	...
	result:=true;
	// файл удален
	if pos('Указанный файл не найден', text) > 0 then
		begin
		SetRetResult('%notfound%');
		exit;
		end;
	// файл не найден
  	if pos('все свободные для закачки слоты заняты', text) > 0 then
		begin
		SetRetResult('');
		exit;
		end;
